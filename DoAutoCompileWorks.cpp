...
set<string> GetFiles(string Path)
{
    ...
    while ((DirentPointer = readdir(DirPointer)) != NULL)
    {
        ...
        else if (DirentPointer->d_type == DT_DIR)
        {
            ...
            if (FolderName == "." || FolderName == ".." || FolderName == "lib" || FolderName == "unfinish")
            {
                continue;
            }
...
int main()
{
    ...
    {
        int PointPos;
        if ((PointPos = sit->find_last_of(".")) == sit->npos)
        {
            remove(sit->c_str());
        }
        else
        {
            string AfterPoint = sit->substr(PointPos + 1);
    ...
    {
        cout << "\r" << CompiledList.size() << "/" << CompileList.size();
    }
    for (int i = 0; i < ThreadCount; i++)
    {
        ThreadList[i].join();
