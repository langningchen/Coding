...
ll query(ll l, ll r, ll k) {
	pair<ll, ll> cnt;
	if (b[l] == b[r]) {
		for (int i = l; i <= r; i++) if (a[i] <= make_pair(k, INF)) cnt = max(cnt, a[i]);
		return cnt.second;
	}
	for (int i = l; b[i] == b[l]; i++) if (a[i] <= make_pair(k, INF)) cnt = max(cnt, a[i]);
	for (int i = r; b[i] == b[r]; i--) if (a[i] <= make_pair(k, INF)) cnt = max(cnt, a[i]);
	for (int j = b[l] + 1; j <= b[r] - 1; j++) {
		pair<ll, ll> tmp = *(upper_bound(as + x[j], as + y[j] + 1, make_pair(k, INF)) - 1); 
		if (tmp <= make_pair(k, INF)) cnt = max(cnt, tmp);
	}
	return cnt.second;
}
int main() {
