...
int main()
{
    ...
    GetDataToFile("https://m.weathercn.com/citysearchajax.do?q=" +
                  URLEncode(IPData["province"].as_string() + " " +
                                IPData["area"].as_string(),
                            true));
    json CityData = json::parse(GetDataFromFileToString());
    if (CityData["listAccuCity"].size() == 0)
    {
        cout << "无法获取城市信息：" << IPData["province"] << " " << IPData["area"] << endl;
        return 0;
    }
    string CityID = CityData["listAccuCity"][0]["key"];
