...
void get_solution(int solution_id, char *work_dir, int lang)
{
	...
	if (lang == 6)
	{ // 从源码中搜索python2字样，失败的结果非零默认python3,成功的结果为0是python2
				   ...
				   char *work_dir, char *outfile, char *userfile, int runner_id)
{
	...
	{
		if (copy_data)
			execute_cmd("/bin/cp '%s' %s/data.in", infile, work_dir); // 如果开启了COPY_DATA则复制测试数据
	}
...
float raw_text_judge(char *infile, char *outfile, char *userfile)
{
	...
	for (int i = 0; i < total; i++)
	{
		...
		if (i == num - 1)
		{
			if (fscanf(out, "%*[^\[][%f] %s", &m[num], ans[num]) != 2)
				return -3;
		}
	}
	fclose(out);
	...
	{
		if (fscanf(user, "%d %s", &num, user_answer) != 2)
			continue;
		if (num > 0 && num <= total)
		{
